...
main = do
    ...
    let
        board = stuck $ parseBoard contents
...
stuck :: Board -> Board
stuck a = R.computeS $ R.traverse a id update
        where
            R.Z R.:. x R.:. y = R.extent a
            update prev ind@(R.Z R.:. i R.:. j) = if ((i==0||i==x-1)&&(j==0||j==y-1)) then True else (prev ind)

step' :: Board -> Board
step' a = R.computeS $ R.traverse a id update
        where
            R.Z R.:. x R.:. y = R.extent a
            update prev (R.Z R.:. i R.:. j) = if ((i==0||i==x-1)&&(j==0||j==y-1)) then True else
                                                    case (prev (R.ix2 i j)) of
                                                        True  -> (2 <= n && n <= 3)
                                                        False -> (n==3)
                where
